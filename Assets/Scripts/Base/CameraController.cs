...
public class CameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //ZOOM
        if (Mouse.current.scroll.ReadValue().y != 0 && !eventSys.IsPointerOverGameObject() && canZoom)
        {
            ...
            if(ActionControl.zoomToMouse)
            {
                ...
                if (GameRaycastObject.instance?.bodyPartScript != null)
                    pivot = GameRaycastObject.instance.bodyPartScript.center;
        ...
        //MOVE
        if ((Mouse.current.middleButton.isPressed || ActionControl.draggingMoveIcon) && !lerping && !movementIsBlocked && canMove)
        {
