...
public class TangibleBodyPart : MonoBehaviour
{
    ...
    private void Start()
    {
        childs = GetComponentsInChildren<TangibleBodyPart>(true).Where(it => !it.nameScript.originalName.Contains(".col")).ToList();
        childs.Remove(this);
    ...
    {
        if (KeyColors.Instance == null)
        {
            Debug.Log("KeyColors script not instantiated");
            return;
        }
